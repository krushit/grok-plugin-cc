...
const MODEL_ALIASES = new Map([["spark", "grok-4.5"]]);
...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/grok-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [prompt]",
...
function ensureGrokAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("Grok CLI is not installed or is missing required runtime support. Install Grok Build so `grok` is on PATH (or set GROK_BIN), then rerun `/grok:setup`.");
  }
}
...
function renderTransferResult(payload) {
  const lines = [
    "Transferred the Claude session into a Grok session with visible turn history.",
...
async function handleCancel(argv) {
  ...
  terminateProcessTree(job.pid ?? Number.NaN, { expectMarker: job.id });

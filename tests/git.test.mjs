...
import { collectReviewContext, resolveReviewTarget, saveTurnBaseline } from "../plugins/grok/scripts/lib/git.mjs";
...
// Regression: the baseline parsed the human-readable porcelain, which QUOTES any
// path git thinks is special. `"réponse.txt"` arrived quotes-and-all, matched
// nothing on disk, hashed to "", and so looked unchanged before and after -- the
// gate silently failing to notice an edit. A literal " -> " in a name broke the
// rename heuristic the same way.
test("turn baseline sees files with awkward names", () => {
  ...
  const awkward = ["réponse.txt", "with space.txt", "a -> b.txt"];
  for (const name of awkward) {
    fs.writeFileSync(path.join(repo, name), "one\n");
  }

  const stateDir = path.join(repo, ".state");
  saveTurnBaseline(repo, stateDir, "s1");
  const before = JSON.parse(
    fs.readFileSync(path.join(stateDir, "turn-baseline-s1.json"), "utf8")
  );

  for (const name of awkward) {
    assert.ok(before.files[name], `baseline should hash ${name}, got ${JSON.stringify(before.files)}`);
    assert.notEqual(before.files[name], "", `${name} hashed to the empty string`);
  }

  // An edit to each must change its hash, which is the whole point of the baseline.
  for (const name of awkward) {
    fs.writeFileSync(path.join(repo, name), "two\n");
  }
  saveTurnBaseline(repo, stateDir, "s2");
  const after = JSON.parse(
    fs.readFileSync(path.join(stateDir, "turn-baseline-s2.json"), "utf8")
  );
  for (const name of awkward) {
    assert.notEqual(after.files[name], before.files[name], `edit to ${name} went unnoticed`);
  }
});

test("turn baseline records a rename under its new path", () => {
  const repo = makeTempDir();
  initGitRepo(repo);
  ...
  // The new path is the record; the original must not be mistaken for one.
  assert.ok(baseline.files["new.txt"], "new path missing from baseline");
  assert.equal(baseline.files["new.txt"] === "", false);
});
